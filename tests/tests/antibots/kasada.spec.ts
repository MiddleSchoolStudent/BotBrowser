...
test('kick', async ({ page }) => {
    ...
    await page.locator('input[name="username"]').pressSequentially(Math.random().toString(36).substring(2), {
        delay: 40,
    });
...
test('twitch', async ({ page }) => {
    ...
    await page.locator('input#signup-username').pressSequentially(Math.random().toString(36).substring(2), {
        delay: 40,
    });
    await page
        .locator('input#password-input')
